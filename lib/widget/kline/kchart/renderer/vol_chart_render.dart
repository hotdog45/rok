...
class VolChartRenderer extends BaseChartRenderer<VolumeEntity> {
  VolChartRenderer(Rect mainRect, double maxValue, double minValue,
      double topPadding, double candleWidth)
      : super(
            chartRect: mainRect,
            maxValue: maxValue,
            minValue: minValue,
  ...
  @override
  void drawChart(Canvas canvas, VolumeEntity lastpoint, VolumeEntity curpoint,
      double curX) {
    drawVolChart(canvas, curpoint, curX);
    if (lastpoint != null) {
      if (lastpoint.MA5Volume != 0) {
        drawLine(canvas, lastpoint.MA5Volume, curpoint.MA5Volume, curX,
            ChartColors.ma5Color);
      }
      if (lastpoint.MA10Volume != 0) {
        drawLine(canvas, lastpoint.MA10Volume, curpoint.MA10Volume, curX,
  ...
  void drawRightText(Canvas canvas, int gridRows) {
    TextSpan span = TextSpan(
        text: "${NumberUtil.volFormat(maxValue)}",
