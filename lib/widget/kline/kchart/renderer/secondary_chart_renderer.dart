...
class SecondaryChartRenderer extends BaseChartRenderer<MACDEntity> {
  ...
  SecondaryChartRenderer(Rect mainRect, double maxValue, double minValue,
      double topPadding, double candleWidth, this.state)
      : super(
            chartRect: mainRect,
            maxValue: maxValue,
            minValue: minValue,
  ...
  @override
  void drawChart(
      Canvas canvas, MACDEntity lastpoint, MACDEntity curpoint, double curX) {
    if (state == SecondaryState.MACD) {
      drawMACD(canvas, lastpoint, curpoint, curX);
    } else if (state == SecondaryState.KDJ && lastpoint != null) {
      if (lastpoint.k != 0)
        drawLine(canvas, lastpoint.k, curpoint.k, curX, ChartColors.kColor);
      if (lastpoint.d != 0)
        drawLine(canvas, lastpoint.d, curpoint.d, curX, ChartColors.dColor);
      if (lastpoint.j != 0)
        drawLine(canvas, lastpoint.j, curpoint.j, curX, ChartColors.jColor);
    } else if (state == SecondaryState.RSI && lastpoint != null) {
      if (lastpoint.rsi != 0)
        drawLine(
            canvas, lastpoint.rsi, curpoint.rsi, curX, ChartColors.rsiColor);
    } else if (state == SecondaryState.WR && lastpoint != null) {
      if (lastpoint.r != 0)
        drawLine(canvas, lastpoint.r, curpoint.r, curX, ChartColors.wrColor);
    }
  }
  ...
  void drawMACD(
    ...
    if (curPoint.macd > 0) {
      canvas.drawRect(
          Rect.fromLTRB(macdCenterX - r, macdY, macdCenterX + r, zeroy),
          chartPaint..color = ChartColors.upColor);
    } else {
      canvas.drawRect(
          Rect.fromLTRB(macdCenterX - r, zeroy, macdCenterX + r, macdY),
          chartPaint..color = ChartColors.dnColor);
    }
    if (lastPoint != null) {
      if (lastPoint.dif != 0) {
        drawLine(
            canvas, lastPoint.dif, curPoint.dif, curX, ChartColors.difColor);
      }
      if (lastPoint.dea != 0) {
        drawLine(
            canvas, lastPoint.dea, curPoint.dea, curX, ChartColors.deaColor);
      }
    }
  }
  ...
  void drawGrid(Canvas canvas, int gridRows, int gridColums) {
    ...
    for (int i = 0; i < gridColums; i++) {
      canvas.drawLine(Offset(i * columnSpace, chartRect.top - topPadding),
          Offset(i * columnSpace, chartRect.bottom), gridPaint);
    }
  }
    ...
    switch (state) {
      ...
      case SecondaryState.WR:
        children.add(TextSpan(
            ...
            style: getTextStyle(ChartColors.wrColor)));
        break;
      default:
        break;
    }
